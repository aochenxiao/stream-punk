...
template<typename T> inline MemberInfo getMemberInfo() {
    ...
    if constexpr (std::is_pointer_v<T>) {
        ...
        if constexpr (TypeID_t<type>::kind == E_type::e_customType || std::is_same_v<type, Base>) {
            ...
            r.serializeCode = "o.write_ptr_with_typeID($(value).value)";
        }
        else {
...
int main_ts(int argc, char** argv) {
    cxxopts::Options options("stream-punk-ts-gen", "Generates TypeScript enum from C++ header");
    options.add_options()
        ("p,path", "Output file path", cxxopts::value<std::string>()->default_value("./stream-punk-data.ts"));
    auto result = options.parse(argc, argv);
    if (result.count("path") == 0) {
        std::cerr << "Error: Missing required argument --path" << std::endl;
    ...
    std::ifstream input("stream-punk.ts");
    if (input.is_open()) {
        outfile << input.rdbuf();
        input.close();
    }
    else {
        std::cerr << "Warning: Could not open stream-punk.ts" << std::endl;
        return -1;
    }
...
# define X_outPutEnumMember(type, name, ...) outfile << "  " << #name << " = " << static_cast<int>(E_type::name) << ",\n";
    ...
    outfile <<
        /*
  readonly ts_au37zuq7832u_id: number;
  private static ts_au37zuq7832u_nextId = 1;
  constructor() { this.id = Base.nextId++; }
        */
        R"(
export class Base {
...
# define X_outputClassCode(typeName__, ...) outfile << genClassCode<typeName__>();
    ...
    outfile.close();
    std::cout << "Successfully generated TypeScript enum to " << output_path << std::endl;
    return 0;
}
