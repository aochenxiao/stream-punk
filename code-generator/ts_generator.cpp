...
template<typename T> inline MemberInfo getMemberInfo() {
    ...
    if constexpr (specialization_of<T, std::variant>) {
        ...
        for (size_t i = 0;i < arr.size(); ++i) {
            ...
            r.tname += arr[i].tname;
            r.deserializeCode += "()=> " + arr[i].deserializeCode;
            std::cout << arr[i].serializeCode << std::endl;
            std::string callStr = "(v)=> " + std::regex_replace(arr[i].serializeCode, reg_value, "v");
            static std::regex v_parmter(R"(\(v\)=>)");
            callStr = std::regex_replace(callStr, v_parmter, "(v:any)=>");
            r.serializeCode += callStr;
        }
        r.deserializeCode += "])";
        r.tname += "]>";
