#pragma once
#include "00-demo-types/customData.hpp"  // 先注册 demo 类型（覆盖库默认的 Xt_CustomType）
...
#include <variant>
#include <tuple>
// 平台检测宏（替代 boost/predef.h，消除对 Boost 的依赖）
// OS
#if defined(_WIN32)
#  define SP_OS_WINDOWS 1
#else
#  define SP_OS_WINDOWS 0
#endif
#if defined(__APPLE__) && defined(__MACH__)
#  include <TargetConditionals.h>
#  if TARGET_OS_IOS
#    define SP_OS_IOS 1
#    define SP_OS_MACOS 0
#  else
#    define SP_OS_IOS 0
#    define SP_OS_MACOS 1
#  endif
#else
#  define SP_OS_MACOS 0
#  define SP_OS_IOS 0
...
#else
#  define SP_OS_LINUX 0
#endif
#if defined(__ANDROID__)
#  define SP_OS_ANDROID 1
#else
#  define SP_OS_ANDROID 0
#endif
...
#  define SP_ARCH_X86_64 1
#else
#  define SP_ARCH_X86_64 0
#endif
#if defined(__i386__) || defined(_M_IX86)
#  define SP_ARCH_X86 1
#else
#  define SP_ARCH_X86 0
#endif
#if defined(__aarch64__) || defined(_M_ARM64)
#  define SP_ARCH_ARM64 1
#else
#  define SP_ARCH_ARM64 0
#endif
#if defined(__arm__) || defined(_M_ARM)
#  define SP_ARCH_ARM 1
#else
#  define SP_ARCH_ARM 0
#endif
#if defined(__ppc__) || defined(__powerpc__) || defined(_M_PPC)
#  define SP_ARCH_PPC 1
#else
#  define SP_ARCH_PPC 0
#endif
// Compiler
#if defined(_MSC_VER)
#  define SP_COMP_MSVC _MSC_VER
#else
#  define SP_COMP_MSVC 0
#endif
#if defined(__clang__)
#  define SP_COMP_CLANG 1
#else
#  define SP_COMP_CLANG 0
#endif
#if defined(__GNUC__) && !defined(__clang__) && !defined(__INTEL_COMPILER)
#  define SP_COMP_GNUC (__GNUC__ * 10000000 + __GNUC_MINOR__ * 100000 + __GNUC_PATCHLEVEL__)
#else
#  define SP_COMP_GNUC 0
#endif
#if defined(__INTEL_COMPILER)
#  define SP_COMP_INTEL __INTEL_COMPILER
#else
#  define SP_COMP_INTEL 0
#endif
...
struct StreamPunkMachineInfo {
    ...
    void init() {
        ...
        if      constexpr (SP_OS_WINDOWS ) os = e_os_win;
        else if constexpr (SP_OS_MACOS   ) os = e_os_mac;
        else if constexpr (SP_OS_LINUX   ) os = e_os_linux;
        else if constexpr (SP_OS_IOS     ) os = e_os_ios;
# if defined(SP_OS_ANDROID)
        else if constexpr (SP_OS_ANDROID ) os = e_os_android;
# endif
        else if constexpr (SP_OS_BSD     ) os = e_os_bsd;
        else os = e_os_unknow;

        if      constexpr (SP_ARCH_X86_64) cpu = e_cpu_X86_64;
        else if constexpr (SP_ARCH_X86   ) cpu = e_cpu_x86;
# if defined(SP_ARCH_ARM64)
        else if constexpr (SP_ARCH_ARM64 ) cpu = e_cpu_arm64;
# endif
        else if constexpr (SP_ARCH_ARM   ) cpu = e_cpu_arm;
        else if constexpr (SP_ARCH_PPC   ) cpu = e_cpu_ppc;
        else cpu = e_cpu_unknow;

        uint32_t compVerRaw = 0;
        if      constexpr (SP_COMP_MSVC  ) {
            compiler = e_compiler_msvc;
            compVerRaw = SP_COMP_MSVC;
        }
        else if constexpr (SP_COMP_CLANG ) {
            compiler = e_compiler_clang;
            compVerRaw = SP_COMP_CLANG;
        }
        else if constexpr (SP_COMP_GNUC  ) {
            compiler = e_compiler_gcc;
            compVerRaw = SP_COMP_GNUC;
        }
        else if constexpr (SP_COMP_INTEL ) {
            compiler = e_compiler_intel;
            compVerRaw = SP_COMP_INTEL;
        }
        else {
...
#pragma pack(pop)
inline O& operator<<(O& s, const StreamPunkMachineInfo& v) { s.w->write(reinterpret_cast<char const*>(&v), sizeof(v)); return s; }
inline I& operator>>(I& s, StreamPunkMachineInfo& v) { s.r->read(reinterpret_cast<char*>(&v), sizeof(v)); return s; }
...
#define Xt_AllBasicTypes(X__) \
    ...
    UseDataJson(AllBasicTypes);
};
REGISTER_JSON_TYPE(AllBasicTypes);
...
#define Xt_TemplateContainer(X__) \
    ...
    UseDataJson(TemplateContainer);
};
REGISTER_JSON_TYPE(TemplateContainer);
...
#define Xt_ComplexTemplateNesting(X__) \
 	...
 	UseDataJson(ComplexTemplateNesting);
};
REGISTER_JSON_TYPE(ComplexTemplateNesting);
...
#define Xt_ComprehensiveContainer(X__) \
    ...
    UseDataJson(ComprehensiveContainer);
};
REGISTER_JSON_TYPE(ComprehensiveContainer);
...
#define Xt_PointerContainer(X__) \
    ...
    UseDataJson(PointerContainer);
};
REGISTER_JSON_TYPE(PointerContainer);
...
#define Xt_Child(X__) \
    ...
    UseDataJsonBase(Child, AllBasicTypes);
};
REGISTER_JSON_TYPE(Child);
...
#define Xt_SelfReferential(X__) \
    ...
    UseDataJson(SelfReferential);
};
REGISTER_JSON_TYPE(SelfReferential);
...
#define Xt_TemplateAndPointer(X__) \
    ...
    UseDataJson(TemplateAndPointer);
};
REGISTER_JSON_TYPE(TemplateAndPointer);
...
#define Xt_InheritanceAndSelfReference(X__) \
    ...
    UseDataJsonBase(InheritanceAndSelfReference, Child);
};
REGISTER_JSON_TYPE(InheritanceAndSelfReference);
...
#define Xt_MegaComplexClass(X__) \
	...
	UseDataJsonBase(MegaComplexClass, Child);
};
REGISTER_JSON_TYPE(MegaComplexClass);
...
#define Xt_SuperComplexContainer(X__) \
	...
	UseDataJson(SuperComplexContainer);
};
REGISTER_JSON_TYPE(SuperComplexContainer);
...
REGISTER_JSON_TYPE(MQTT); // struct MQTT
...
#define Xt_PointerDemo(X__) \
    ...
    UseData(PointerDemo);
    UseDataJson(PointerDemo);
};
REGISTER_JSON_TYPE(PointerDemo);
...
#define Xt_ContainerDemo(X__) \
    ...
    UseDataJson(ContainerDemo);
};
REGISTER_JSON_TYPE(ContainerDemo);
...
#define Xt_NetworkSystem(X__) \
    ...
    UseDataJson(NetworkSystem);
};
REGISTER_JSON_TYPE(NetworkSystem);
...
#define Xt_NetworkDevice(X__) \
    ...
    UseDataJsonBase(NetworkDevice,Device);
};
REGISTER_JSON_TYPE(NetworkDevice);
...
#define Xt_TemperatureSensor(X__) \
    ...
    UseDataJsonBase(TemperatureSensor, Sensor);
...
REGISTER_JSON_TYPE(TemperatureSensor);
...
#define Xt_SmartHomeSystem(X__) \
    ...
    UseDataJson(SmartHomeSystem);
};
REGISTER_JSON_TYPE(SmartHomeSystem);
...
#define Xt_MultiLevelContainer(X__) \
    ...
    UseDataJson(MultiLevelContainer);
};
REGISTER_JSON_TYPE(MultiLevelContainer);
...
#define Xt_SptrTest(X__) \
    ...
    SptrTest() = default;
    UseData(SptrTest);
    UseDataJson(SptrTest);
};
REGISTER_JSON_TYPE(SptrTest);
...
#define Xt_MousePosition(X__) \
    ...
    UseDataJson(MousePosition);
};
REGISTER_JSON_TYPE(MousePosition);

// 影子模式增量更新测试专用结构体
struct ShadowTestData : public Base {
#define Xt_ShadowTestData(X__) \
    X__(std::vector<i32>, numbers, {}) \
    X__(std::map<i32 DH std::string>, items, {}) \
    X__(std::optional<i32>, optVal, std::nullopt) \
    X__(MousePosition, pos, {})

    ShadowTestData() = default;
    UseData(ShadowTestData);
};
