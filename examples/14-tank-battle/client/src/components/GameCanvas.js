...
import { PlayerInput } from '../stream-punk-data';
const CANVAS_W = 800;
const CANVAS_H = 600;
const INPUT_INTERVAL = 50; // ms
export default function GameCanvas({ ws, playerId, onBack }) {
    ...
    return (_jsxs("div", { style: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }, children: [_jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: '16px' }, children: [_jsx("span", { style: { color: '#00d2ff', fontWeight: 'bold' }, children: "P1: WASD + \u7A7A\u683C" }), _jsx("span", { style: { color: '#e94560', fontWeight: 'bold' }, children: "P2: \u65B9\u5411\u952E + \u56DE\u8F66" }), _jsx("button", { onClick: onBack, style: {
                            padding: '6px 16px',
                            background: '#333',
                            color: '#fff',
                            ...
                            borderRadius: '4px',
                            cursor: 'pointer',
                        }, children: "\u8FD4\u56DE" })] }), _jsx("canvas", { ref: canvasRef, width: CANVAS_W, height: CANVAS_H, style: { border: '2px solid #333', borderRadius: '4px', background: '#1a1a2e' } })] }));
}
