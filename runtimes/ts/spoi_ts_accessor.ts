...
export interface SpoiAccessor {
    fieldCount(): number;
    getField(obj: any, idx: number): any;
    setField(obj: any, idx: number, val: any): void;
}
...
export function deserializeValue(data: Uint8Array): unknown {
    if (data.length < 4) return null;
    const view = new DataView(data.buffer, data.byteOffset, data.length);
    const typeId = view.getUint32(0, true);
    const valueBytes = data.slice(4);
    if (typeId === TypeId.U8)   return valueBytes[0];
    if (typeId === TypeId.U16)  return view.getUint16(4, true);
    if (typeId === TypeId.U32)  return view.getUint32(4, true);
    if (typeId === TypeId.U64)  return view.getBigUint64(4, true);
    if (typeId === TypeId.I8)   return view.getInt8(4);
    if (typeId === TypeId.I16)  return view.getInt16(4, true);
    if (typeId === TypeId.I32)  return view.getInt32(4, true);
    if (typeId === TypeId.I64)  return view.getBigInt64(4, true);
    if (typeId === TypeId.F32)  return view.getFloat32(4, true);
    if (typeId === TypeId.F64)  return view.getFloat64(4, true);
...
class SpoiTestPlayerAccessor implements SpoiAccessor {
    ...
    setField(obj: any, idx: number, val: any): void {
        const o = obj as any;
        switch (idx) {
            case 0: o.name = val; break;
            case 1: o.hp = val; break;
            case 2: o.level = val; break;
            case 3: o.posX = val; break;
            default: throw new Error(`invalid field index for SpoiTestPlayer: ${idx}`);
        }
    }
}
...
class SpoiTestStateAccessor implements SpoiAccessor {
    ...
    setField(obj: any, idx: number, val: any): void {
        const o = obj as any;
        switch (idx) {
            case 0: o.tick = val; break;
            case 1: o.currentMap = val; break;
...
class SpoiWorldAccessor implements SpoiAccessor {
    ...
    setField(obj: any, idx: number, val: any): void {
        const o = obj as any;
        switch (idx) {
            case 0: o.worldName = val; break;
            case 1: o.tick = val; break;
...
export const SpoiAccessorRegistry: Map<string, SpoiAccessor> = new Map([
