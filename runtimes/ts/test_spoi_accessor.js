...
function typedOperand(typeId, valueBytes) {
...
function setIntField(path, value) {
    return inst(SET, path, typedOperand(spoi_ts_accessor_1.TypeId.I32, i32Bytes(value)));
}
function setStrField(path, value) {
    return inst(SET, path, typedOperand(spoi_ts_accessor_1.TypeId.STRING, strBytes(value)));
}
function setBoolField(path, value) {
    return inst(SET, path, typedOperand(spoi_ts_accessor_1.TypeId.BOOL, [value ? 1 : 0]));
}
function pipeOp(path) {
    return inst(PIPE, path || [], new Uint8Array());
}
function selectOp(path) {
...
suite('TypeId 常量', () => {
    test('整数类型', () => {
        assert.strictEqual(spoi_ts_accessor_1.TypeId.U8, 26);
        assert.strictEqual(spoi_ts_accessor_1.TypeId.U16, 27);
        assert.strictEqual(spoi_ts_accessor_1.TypeId.U32, 28);
        assert.strictEqual(spoi_ts_accessor_1.TypeId.U64, 29);
    });
    test('有符号整数类型', () => {
        assert.strictEqual(spoi_ts_accessor_1.TypeId.I8, 30);
        assert.strictEqual(spoi_ts_accessor_1.TypeId.I16, 31);
        assert.strictEqual(spoi_ts_accessor_1.TypeId.I32, 32);
        assert.strictEqual(spoi_ts_accessor_1.TypeId.I64, 33);
    });
    test('浮点类型', () => {
        assert.strictEqual(spoi_ts_accessor_1.TypeId.F32, 34);
        assert.strictEqual(spoi_ts_accessor_1.TypeId.F64, 35);
    });
    test('字符串与布尔类型', () => {
        assert.strictEqual(spoi_ts_accessor_1.TypeId.STRING, 9);
        assert.strictEqual(spoi_ts_accessor_1.TypeId.BOOL, 40);
    });
    test('CUSTOM 类型', () => {
        assert.strictEqual(spoi_ts_accessor_1.TypeId.CUSTOM, 0);
    });
});
// =============================== 2. deserializeValue 函数测试 ===============================
suite('deserializeValue', () => {
    test('U8', () => {
        const data = typedOperand(spoi_ts_accessor_1.TypeId.U8, u8Bytes(42));
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(data), 42);
    });
    test('U8 最大值', () => {
        const data = typedOperand(spoi_ts_accessor_1.TypeId.U8, u8Bytes(255));
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(data), 255);
    });
    test('U16', () => {
        const buf = new ArrayBuffer(4 + 2);
        const view = new DataView(buf);
        view.setUint32(0, spoi_ts_accessor_1.TypeId.U16, true);
        view.setUint16(4, 0x1234, true);
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(new Uint8Array(buf)), 0x1234);
    });
    test('U32', () => {
        const buf = new ArrayBuffer(4 + 4);
        const view = new DataView(buf);
        view.setUint32(0, spoi_ts_accessor_1.TypeId.U32, true);
        view.setUint32(4, 0xDEADBEEF, true);
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(new Uint8Array(buf)), 0xDEADBEEF);
    });
    test('U64', () => {
        const buf = new ArrayBuffer(4 + 8);
        const view = new DataView(buf);
        view.setUint32(0, spoi_ts_accessor_1.TypeId.U64, true);
        view.setBigUint64(4, 0x123456789abcdef0n, true);
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(new Uint8Array(buf)), 0x123456789abcdef0n);
    });
    test('I8 正数', () => {
        const buf = new ArrayBuffer(4 + 1);
        const view = new DataView(buf);
        view.setUint32(0, spoi_ts_accessor_1.TypeId.I8, true);
        view.setInt8(4, 100);
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(new Uint8Array(buf)), 100);
    });
    test('I8 负数', () => {
        const buf = new ArrayBuffer(4 + 1);
        const view = new DataView(buf);
        view.setUint32(0, spoi_ts_accessor_1.TypeId.I8, true);
        view.setInt8(4, -128);
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(new Uint8Array(buf)), -128);
    });
    test('I16', () => {
        const buf = new ArrayBuffer(4 + 2);
        const view = new DataView(buf);
        view.setUint32(0, spoi_ts_accessor_1.TypeId.I16, true);
        view.setInt16(4, -1000, true);
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(new Uint8Array(buf)), -1000);
    });
    test('I32', () => {
        const buf = new ArrayBuffer(4 + 4);
        const view = new DataView(buf);
        view.setUint32(0, spoi_ts_accessor_1.TypeId.I32, true);
        view.setInt32(4, -123456, true);
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(new Uint8Array(buf)), -123456);
    });
    test('I64', () => {
        const buf = new ArrayBuffer(4 + 8);
        const view = new DataView(buf);
        view.setUint32(0, spoi_ts_accessor_1.TypeId.I64, true);
        view.setBigInt64(4, -1234567890123n, true);
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(new Uint8Array(buf)), -1234567890123n);
    });
    test('F32', () => {
        const buf = new ArrayBuffer(4 + 4);
        const view = new DataView(buf);
        view.setUint32(0, spoi_ts_accessor_1.TypeId.F32, true);
        view.setFloat32(4, 3.14, true);
        assert.ok(Math.abs((0, spoi_ts_accessor_1.deserializeValue)(new Uint8Array(buf)) - 3.14) < 0.001);
    });
    test('F64', () => {
        const buf = new ArrayBuffer(4 + 8);
        const view = new DataView(buf);
        view.setUint32(0, spoi_ts_accessor_1.TypeId.F64, true);
        view.setFloat64(4, 3.141592653589793, true);
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(new Uint8Array(buf)), 3.141592653589793);
    });
    test('String', () => {
        const text = 'hello world';
        const bytes = strBytes(text);
        const data = typedOperand(spoi_ts_accessor_1.TypeId.STRING, bytes);
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(data), text);
    });
    test('String 空串', () => {
        const data = typedOperand(spoi_ts_accessor_1.TypeId.STRING, []);
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(data), '');
    });
    test('String 中文', () => {
        const text = '你好世界';
        const bytes = strBytes(text);
    ...
    test('Bool true', () => {
        const data = typedOperand(spoi_ts_accessor_1.TypeId.BOOL, [1]);
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(data), true);
    });
    test('Bool false', () => {
        const data = typedOperand(spoi_ts_accessor_1.TypeId.BOOL, [0]);
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(data), false);
    });
    test('空数据（长度不足）', () => {
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(new Uint8Array(0)), null);
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(new Uint8Array(1)), null);
        assert.strictEqual((0, spoi_ts_accessor_1.deserializeValue)(new Uint8Array(3)), null);
    });
});
// =============================== 3. Accessor 测试 ===============================
suite('SpoiTestPlayerAccessor', () => {
    const accessor = spoi_ts_accessor_1.SpoiAccessorRegistry.get('SpoiTestPlayer');
    test('fieldCount', () => {
...
suite('SpoiTestStateAccessor', () => {
    const accessor = spoi_ts_accessor_1.SpoiAccessorRegistry.get('SpoiTestState');
    test('fieldCount', () => {
...
suite('SpoiItemAccessor', () => {
    const accessor = spoi_ts_accessor_1.SpoiAccessorRegistry.get('SpoiItem');
    test('fieldCount', () => {
...
suite('SpoiInventoryAccessor', () => {
    const accessor = spoi_ts_accessor_1.SpoiAccessorRegistry.get('SpoiInventory');
    test('fieldCount', () => {
        assert.strictEqual(accessor.fieldCount(), 3);
    });
    test('getField', () => {
...
suite('SpoiCharacterAccessor', () => {
    const accessor = spoi_ts_accessor_1.SpoiAccessorRegistry.get('SpoiCharacter');
    test('fieldCount', () => {
...
suite('SpoiWorldAccessor', () => {
    const accessor = spoi_ts_accessor_1.SpoiAccessorRegistry.get('SpoiWorld');
    test('fieldCount', () => {
        assert.strictEqual(accessor.fieldCount(), 3);
    });
    test('getField', () => {
...
suite('SpoiAccessorRegistry', () => {
    test('包含所有 6 个类型', () => {
        assert.strictEqual(spoi_ts_accessor_1.SpoiAccessorRegistry.size, 6);
        assert.ok(spoi_ts_accessor_1.SpoiAccessorRegistry.has('SpoiTestPlayer'));
        assert.ok(spoi_ts_accessor_1.SpoiAccessorRegistry.has('SpoiTestState'));
        assert.ok(spoi_ts_accessor_1.SpoiAccessorRegistry.has('SpoiItem'));
        assert.ok(spoi_ts_accessor_1.SpoiAccessorRegistry.has('SpoiInventory'));
        assert.ok(spoi_ts_accessor_1.SpoiAccessorRegistry.has('SpoiCharacter'));
        assert.ok(spoi_ts_accessor_1.SpoiAccessorRegistry.has('SpoiWorld'));
    });
    test('每个 entry 都是 SpoiAccessor 实例', () => {
        for (const [name, acc] of spoi_ts_accessor_1.SpoiAccessorRegistry) {
...
suite('Executor 集成（v2 访问器驱动）', () => {
    const executor = new spoi_executor_1.SpoiExecutor(spoi_ts_accessor_1.SpoiAccessorRegistry);
    test('SET 写入字段', () => {
    ...
    test('PIPE 单个对象 → SINGLE', () => {
        ...
        assert.strictEqual(result.value.name, 'Solo');
    });
    test('PIPE → SELECT 字段', () => {
        const players = [
    ...
    test('SET 后 PIPE 读取', () => {
        ...
        assert.strictEqual(result.value.hp, 777);
    });
