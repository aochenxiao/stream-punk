...
import { SpoiExecutor, Op, ResultType, PATH_DEREF } from './spoi_executor';
import { TypeId } from './spoi_ts_accessor';
...
class Player {
  ...
  health: number = 0;
  items: string[] = [];
  constructor(init?: Partial<Player>) { Object.assign(this, init); }
}

// 类型注册表（旧格式：字段名数组，兼容旧测试）
const TYPE_REGISTRY: Map<string, string[]> = new Map([
...
function buildOperandInt(value: number): Uint8Array {
...
function buildOperandStr(value: string): Uint8Array {
...
function setInt(path: number[], value: number) { return inst(Op.SET, path, buildOperandInt(value)); }
function setStr(path: number[], value: string) { return inst(Op.SET, path, buildOperandStr(value)); }
function pipe(path?: number[]) { return inst(Op.PIPE, path || [], new Uint8Array()); }
function filterGt(memberIdx: number, value: number) {
...
function select(path: number[]) { return inst(Op.SELECT, path, new Uint8Array()); }
function sort(path?: number[]) { return inst(Op.SORT, path || [], new Uint8Array()); }
function take(n: number) { return inst(Op.TAKE, [], u32(n)); }
function drop(n: number) { return inst(Op.DROP, [], u32(n)); }
...
suite('路径导航', () => {
  ...
  test('array index', () => {
    ...
    assert.strictEqual((executor.navigate(players, [0]) as Player).name, "Alice");
    assert.strictEqual((executor.navigate(players, [1]) as Player).name, "Bob");
  });
  ...
  test('deref pointer', () => {
    ...
    assert.strictEqual((executor as any).navStep(ref, PATH_DEREF), "hello");
  });
});
...
suite('读操作', () => {
  ...
  test('pipe', () => {
    ...
    assert.strictEqual((result.value as any[]).length, 5);
  });
  ...
  test('filter gt', () => {
    ...
    assert.strictEqual((result.value as any[]).length, 2); // Bob(20), Dave(20)
  });
  ...
  test('reverse', () => {
    ...
    const values = result.value as Player[];
...
suite('聚合', () => {
  ...
  test('any true', () => {
    ...
    const data = buildStream([pipe(), inst(Op.ANY, [], op), exec()]);
    const result = executor.execute(players, data);
  ...
  test('all true', () => {
    ...
    const data = buildStream([pipe(), inst(Op.ALL, [], op), exec()]);
    const result = executor.execute(players, data);
    assert.strictEqual(result.value, true);
  });
  ...
  test('find', () => {
    ...
    const data = buildStream([pipe(), inst(Op.FIND, [], op), exec()]);
    const result = executor.execute(players, data);
    assert.strictEqual((result.value as Player).name, "Bob");
  });
...
suite('完整管道', () => {
  ...
  test('write then read', () => {
    ...
    assert.strictEqual((result.value as Player).name, "Zelda");
  });
});
